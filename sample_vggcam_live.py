...
# -*- coding:utf-8 -*-
"""
# This sample uses a UFF VGGCAM model to create a TensorRT Inference Engine
...
class net():
    def __init__(self,model_file):
        ...
        self.engine = build_engine(self._model_file)
        self._inputs, self._outputs, self._bindings, self._stream = common.allocate_buffers(self.engine)
    ...
    def predict(self,img):
        ...
        [output] = common.do_inference(self.context, bindings=self._bindings, inputs=self._inputs, outputs=self._outputs, stream=self._stream)
...
#    parser.add_argument("--camera", type=str, default="0", help="index of the MIPI CSI camera to use (NULL for CSI camera 0), or for VL42 cameras the /dev/video node to use (e.g. /dev/video0).  By default, MIPI CSI camera 0 will be used.")
    ...
    print("model file: ",model_file)
    with build_engine(model_pathe) as engine:
        # Build an engine, allocate buffers and create a stream.
        inputs, outputs, bindings, stream = common.allocate_buffers(engine)
        with engine.create_execution_context() as context:
            while True:
                _,img = cap.read()
                img2 = cv2.cvtColor(cv2.resize(img,CONFIG.INPUT_SHAPE[1:]),cv2.COLOR_BGR2RGB)
